# -*- coding: utf-8 -*-
"""Kayque_Estevão_de_Queiroga_89632_ Atividade_QA_Banco_do_Stark.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1IkcmYhAQJfagvezTA-lsWHBMjXRr7ktM
...
**Pré-condição:** Conta "tony" criada e com saldo positivo

**Passos:**  
1.  executar `banco.depositar('tony', 0)`

**Resultado esperado:**  O sistema deve exibir uma mensagem de **erro** e não alterar o saldo

**Resultado obtido:**  O sistema aceitou o depósito e atualizou o saldo
...
**ID:** CT-002  
**Título/Objetivo:**  Depósito com valor negativo
...
**Resultado esperado:**  O sistema deve exibir uma mensagem de **erro** ex: Valor de depósito deve ser maior que zero" e não alterar o saldo.

**Resultado obtido:** O sistema aceitou o depósito e reduziu o saldo (comportamento errado, não pode).
...
**ID:** CT-003  
**Título/Objetivo:**  Validar se o sistema rejeita saque com valor acima do saldo

**Pré-condição:** Conta "tony" criada e com saldo de 700

**Passos:**  
1. banco.sacar('tony', 100000)

**Resultado esperado:** O sistema deve exibir "ERRO - Saldo insuficiente" e não alterar o saldo.

**Resultado obtido:** O sistema mostrou que a mensagem de erro devido o saldo atual ser insuficiente para ser sacado
...
**Descrição:**  O sistema aceita depósitos com valor zero, o que viola a regra de negócio que exige que o valor do depósito seja maior que zero.

**Passos para reproduzir:**  
1.  Criar uma conta (ex: "tony")
...
**Resultado obtido:** O sistema aceitou o depósito e atualizou o saldo  
**Evidência:**  OK - Depósito realizado: 0
**Impacto/Risco:**  Inconsistência no saldo e extrato.
...
**Descrição:** O sistema aceita depósitos com **valor negativo**, reduzindo o saldo da conta de forma errada, o que não deveria ser permitido.  
...
**Impacto/Risco:**  Saldo da conta pode ficar negativo ou inconsistente.
Viola as regras de depósito
...
**Descrição:** O sistema permite a existência de uma conta com o nome do titular vazio, o que viola a regra de negócio 'Titular não pode ser vazio (nem só espaços)'

**Passos para reproduzir:**  
1.  Executar `banco.transferir('pepper', '', 100)` Transferencia para uma conta vazia.

2.  Executar `banco.saldo_atual('')` Contultar saldo da conta vazia

**Resultado esperado:** O sistema deveria exibir uma mensagem de erro ('ERRO - Titular inválido') não deveria permitir a criação da conta

**Resultado obtido:** O sistema realizou a transferência para o titular vazio e deu um saldo a ele, tratando-o como uma conta válida.

**Evidência:** OK - Transferência realizada: 100
SALDO  = 100.0

**Impacto/Risco:**  Problema grave nos dados do banco e falha de segurança ao permitir operações com um identificador de conta inválido.
...
# banco.depositar('tony', 900)
#banco.depositar('tony', -100)
# banco.sacar('tony', 10000)
# criando conta sem nome
banco.transferir('pepper', '', 100)
banco.saldo_atual('')
